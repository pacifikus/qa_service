...
def read_posts(posts_filepath):
    """
    The read_posts function reads the posts.xml file and returns a dataframe
    with three columns: post_type_id, title, id. The function only keeps rows
    where post_type_id is equal to 1.
...
def pack_embeddings_with_data(embeddings, posts):
    """
    The pack_embeddings_with_data function takes in a list of embeddings
    and a dataframe of posts. It returns a list of tuples, where each tuple
    contains the post id, title, and embedding.

    :param embeddings: Embeddings for posts
    :param posts: Dataframe with title and id of each post
    :return: A list of tuples, where each tuple contains the index,
     title and embedding for a post
    """
    res = []
...
def compute_embeddings(config_path):
    """
    The compute_embeddings function computes the embeddings for a given
    dataset of post titles. It loads the model from a file, and uses it to
    compute embeddings for each title in the dataset.
    ...
    embeddings_filepath = (
        Path(config["data"]["processed_path"]) / config["data"]["embeddings_filename"]
    )
