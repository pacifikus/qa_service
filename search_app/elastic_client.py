...
class ElasticClient:
    ...
    def resp_msg(self, msg, resp, throw=True):
        ...
        if resp.status_code >= 400 and throw:
            raise RuntimeError(resp.text)

    def delete_index(self, index):
        resp = self.elastic.indices.delete(
            index=index, ignore=[400, 404], ignore_unavailable=True
        )
    ...
    def index_documents(self, index, docs_dict):
        def bulk_docs(docs):
            for doc in docs:
                if "id" not in doc:
                    raise ValueError("Expecting docs to have field 'id'")
                add_command = {
                    "_index": index,
                    "_id": doc["id"],
                    "_text": doc["text"],
                    "_post_id": doc["post_id"],
                    "_source": doc,
                }
                yield add_command

        resp = elasticsearch.helpers.bulk(
            self.elastic, bulk_docs(docs_dict), chunk_size=100
        )
    ...
    def query(self, index, query):
        ...
        self.resp_msg(
            msg="Searching {index} - {str(query)[:15]}", resp=SearchResponse(resp)
        )
